...
let sliderSlider = new Swiper('.slider', {
	...
	//Стартовый слайд
	initialSlide: 1, //отсчетидет от 0
...
let menuArrows = document.querySelectorAll('.menu__link_arrow');
if(menuArrows){
...
document.documentElement.addEventListener("click", function (e){
	if(!e.target.closest('.menu__link_arrow')){
		for (let index = 0; index < menuArrows.length; index++) {
			const menuArrow = menuArrows[index];
...
//=================
// arrow-up
const scrollArrow = document.querySelector('._arrow-up');
if(scrollArrow) {
		const scrollArrow = document.querySelector('._arrow-up');
		scrollArrow.onclick = () => {
		window.scrollTo(0, 0);
...
//=================
// btn-more
var textMore = document.querySelector("._text-more");
var btnMore = document.querySelector("._btn-more");
...
const menuLinks = document.querySelectorAll('._goto');
if (menuLinks.length > 0) {
	...
	function onMenuLinkClick(e) {
		...
		if(menuLink.dataset.goto && document.querySelector(menuLink.dataset.goto)) {
			const target_block_class = document.querySelector(menuLink.dataset.goto);
			const target_block = target_block_class.getBoundingClientRect().top + pageYOffset - document.querySelector('header').offsetHeight;
